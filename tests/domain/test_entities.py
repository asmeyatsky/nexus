...
class TestAccount:
    ...
    def test_account_is_frozen(self):
        from domain.entities.account import Account
        account = Account.create(
            id="acc-1", name="Test", industry="technology",
...
class TestActivity:
    ...
    def test_complete_activity(self):
        from domain.entities.activity import Activity, ActivityType, ActivityStatus
        activity = Activity.create(
            id="act-1", activity_type=ActivityType.TASK,
            subject="Task", description="Do it",
            owner_id="u1", related_entity_type="lead",
            related_entity_id="l1", org_id="org-1",
        )
...
class TestCampaign:
    def test_create_campaign(self):
        ...
        campaign = Campaign.create(
            id="camp-1", name="Q1 Email Blast",
            campaign_type=CampaignType.EMAIL,
            budget=10000.0, currency="USD",
            owner_id="user-1", org_id="org-1",
        )
    ...
    def test_activate_campaign(self):
        ...
        campaign = Campaign.create(
            ...
            budget=5000.0, currency="USD",
            owner_id="u1", org_id="org-1",
        )
...
class TestCSAT:
    def test_create_csat(self):
        ...
        survey = CSATSurvey.create(
            id="csat-1", case_id="case-1",
            contact_id="ct-1", score=4,
            comment="Good support", org_id="org-1",
        )
    ...
    def test_invalid_score(self):
        ...
        with pytest.raises(ValueError):
            CSATSurvey.create(
                id="csat-1", case_id="case-1",
                contact_id="ct-1", score=6,
                comment="", org_id="org-1",
            )
...
class TestHealthScore:
    def test_calculate_health_score(self):
        ...
        score = AccountHealthScore.calculate(
            id="hs-1", account_id="acc-1",
            engagement_score=80, revenue_score=90,
            support_score=70, org_id="org-1",
        )
...
class TestRelationship:
    def test_create_relationship(self):
        ...
        rel = Relationship.create(
            id="rel-1",
            from_entity_type="account", from_entity_id="acc-1",
            to_entity_type="account", to_entity_id="acc-2",
            relationship_type=RelationshipType.PARTNER,
            strength=8, org_id="org-1",
        )
...
class TestProduct:
    def test_create_product(self):
        ...
        product = Product.create(
            id="prod-1", name="CRM License",
            code="CRM-ENT", description="Enterprise license",
            family="Software", unit_price=99.99,
            currency="USD", org_id="org-1",
        )
...
class TestQuote:
    def test_create_quote_with_line_items(self):
        ...
        items = [
            QuoteLineItem(id="li-1", product_id="p1", product_name="License",
                          quantity=10, unit_price=100.0, discount_percent=10),
        ]
        quote = Quote.create(
            id="q-1", opportunity_id="opp-1", name="Q1 Quote",
...
class TestKnowledgeArticle:
    def test_create_and_publish(self):
        ...
        article = KnowledgeArticle.create(
            id="ka-1", title="How to reset password",
            body="Steps to reset...", category="self-service",
            author_id="u1", org_id="org-1",
        )
